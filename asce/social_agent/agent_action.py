...
from asce.social_platform.channel import Channel
from asce.social_platform.typing import ActionType
...
class SocialAction:
    ...
    async def follow(self, followee_id: int, step_number: int = None):
        r"""Follow a user.
        ...
        Args:
            ...
            step_number (int, optional): The simulation step number when this follow action occurs.
                Default is None, which will use the current step from the environment.
        ...
        """
        # 如果提供了step_number，将其作为消息的一部分传递
        if step_number is not None:
            message = (followee_id, step_number)
        else:
            message = followee_id
        return await self.perform_action(message, ActionType.FOLLOW.value)
    ...
    async def reply_to_comment(self, comment_id: int, content: str):
        ...
        返回:
            dict: 包含操作结果的字典
        """
        reply_message = (comment_id, content)
        return await self.perform_action(reply_message, ActionType.REPLY_COMMENT.value)

    async def share_comment(self, comment_id: int):
        """分享评论
        ...
        返回:
            dict: 包含操作结果的字典
        """
        return await self.perform_action(comment_id, ActionType.SHARE_COMMENT.value)

    async def get_trending_comments(self, limit: int = 2):
        """获取热门评论
        ...
        返回:
            dict: 包含热门评论的字典
        """
        return await self.perform_action(limit, ActionType.VIEW_TRENDING_COMMENTS.value)

    async def update_think(self, agent_id: int, step_number: int, sub_step_number: int, action_name: str, content: str, reason: str, cognitive_state: dict, post_id: int = None):
        """
        更新智能体思考记录
        ...
        参数:
            ...
            sub_step_number (int): 子步骤编号，用于区分同一步骤内的多个行为
            action_name (str): 动作名称
            content (str): 思考内容
            reason (str): 思考原因
            cognitive_state (dict): 认知状态
            post_id (int, optional): 相关帖子ID
        ...
        if isinstance(cognitive_state, dict):
            cognitive_state_json = json.dumps(cognitive_state, ensure_ascii=False)
        else:
            cognitive_state_json = str(cognitive_state)
        ...
        message = {
            ...
            "content": content,
            "reason": reason,
            "cognitive_state": cognitive_state_json
        }


        # 发送消息到平台
        return await self.perform_action(message, "update_think")
